...
/**
 * Frozen v102 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v101 would
 * silently change what v102 asserts.
 *
 * Delta from v101: a top-level `wordBook` section holding the enabled switch and
 * the Notion connection (integration token, target database, field mapping).
 * The saved words live in IndexedDB, so no fixture can carry them and no
 * migration touches them. Every existing profile arrives with the shipped
 * defaults.
 */
