...
/**
 * Frozen v103 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v102 would
 * silently change what v103 asserts.
 *
 * Delta from v102: a top-level `learningMode` section holding the feature switch
 * (off as shipped — the marks need an imported dictionary), the difficulty
 * profile, the display controls, the per-site exclusions and the per-page mark
 * cap. The dictionary itself is a multi-megabyte artifact in extension storage,
 * so no fixture can carry it and no migration touches it. Every existing profile
 * arrives with the shipped defaults.
 */
