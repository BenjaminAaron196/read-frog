...
/**
 * Frozen v104 fixtures. Written out in full on purpose: an example file that
 * spreads the previous version is not a snapshot, because editing v103 would
 * silently change what v104 asserts.
 *
 * Delta from v103: three fields inside the existing `learningMode` section —
 * the word card's AI switch and per-page request cap, the `minTier` display
 * quiet mode, and the dictionary source list the one-click download reads.
 * Nothing else moved, and no user data is touched by the migration.
 */
